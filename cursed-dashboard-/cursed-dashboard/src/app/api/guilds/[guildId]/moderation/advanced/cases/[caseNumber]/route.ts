...
import { advancedCaseOperationSchema } from "@/lib/validation/moderation-advanced";
import type { AdvancedCaseOperation } from "@/types/moderation-advanced";
import type { ModerationCase, ModerationStats } from "@/types/moderation";
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  let operation: AdvancedCaseOperation;
  try {
    operation = advancedCaseOperationSchema.parse(bodyResult.body);
  } catch (error) {
  ...
  try {
    ...
    }>(`guilds/${guildId}/moderation/advanced/cases/${caseNumber}`, {
      method: "PATCH",
      headers: {
    ...
    return botApiErrorResponse(error, "Could not update the moderation case.");
  }
}
