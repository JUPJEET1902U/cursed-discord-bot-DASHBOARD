...
export function DiscordSignInButton({ callbackUrl, className }: DiscordSignInButtonProps) {
  ...
  return (
    <Button
      ...
      onClick={() => {
        setIsLoading(true);
        // next-auth/react's signIn redirects the browser to Discord's
        // authorize screen. Client secret and token exchange never touch
        // this code path — that happens server-side in the Auth.js route
        // handler once Discord redirects back with a code.
        void signIn("discord", { callbackUrl });
      }}
    >
      <DiscordMark />
      {isLoading ? "Redirecting to Discord…" : "Sign in with Discord"}
    </Button>
  );
}
